...
<body>
    <header class="bg-dark py-2 py-md-3">
        <div class="container d-flex flex-column flex-md-row align-items-center justify-content-between gap-2 gap-md-1">
            <a class="text-light text-decoration-none fs-4" href="/">НА ГЛАВНУЮ</a>
            ...
            <div>
                <a class="btn" href="/cart" title="Корзина">
                ...
                @if (Auth::check())
                <a href="{{ route('profile.logout') }}" class="btn" title="Выйти">
                ...
                @else
                <a class="btn" href="/register" title="Войти">
                    <svg width="35px" height="35px" viewBox="0 0 24 24" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
...
<script>
    ...
    $('.card-close').click(function () {
        var write_off_bonuses = $('.form-check input').prop('checked');
        var url = '/cart/close?write_off_bonuses=' + (write_off_bonuses ? 'true' : 'false');
        window.location.href = url;
    });

    // $('.card-close').click(function () {
    //     write_off_bonuses = $('.form-check input').prop('checked')
    //     // alert(write_off_bonuses)
    //     $.ajax({
    //         type: 'GET',
    //         url: '/cart/close',
    //         headers: {
    ...
    //         },
    //         data: {
    //             'write_off_bonuses': write_off_bonuses
    //         },
    //         success: function(response) {
    //             // console.log(response)
    //             window.location.href = '/cart/close'
    //         }
    //     })
    // })
</script>
